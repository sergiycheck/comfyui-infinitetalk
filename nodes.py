    ...
    def addWeighted(self, conditioning_to, conditioning_from, conditioning_to_strength):
        ...
        for i in range(len(conditioning_to)):
            ...
            if t0.shape[1] < t1.shape[1]:
                t0 = torch.cat(
                    [t0] + [torch.zeros((1, (t1.shape[1] - t0.shape[1]), t1.shape[2]))],
                    dim=1,
                )

            tw = torch.mul(t1, conditioning_to_strength) + torch.mul(
                t0, (1.0 - conditioning_to_strength)
            )
            t_to = conditioning_to[i][1].copy()
            if pooled_output_from is not None and pooled_output_to is not None:
                t_to["pooled_output"] = torch.mul(
...
class VAEDecodeTiled:
    ...
    def decode(
        self, vae, samples, tile_size, overlap=64, temporal_size=64, temporal_overlap=8
    ):
...
class VAEEncodeTiled:
    ...
    def encode(
        self, vae, pixels, tile_size, overlap, temporal_size=64, temporal_overlap=8
    ):
        t = vae.encode_tiled(
            pixels,
            tile_x=tile_size,
...
class StyleModelApply:
    ...
    def apply_stylemodel(
        self, conditioning, style_model, clip_vision_output, strength, strength_type
    ):
        cond = (
        ...
        for t in conditioning:
            ...
            if "attention_mask" in keys or (
                strength_type == "attn_bias" and strength != 1.0
            ):
                # math.log raises an error if the argument is zero
                # torch.log returns -inf, which is what we want
                attn_bias = torch.log(
                    torch.Tensor([strength if strength_type == "attn_bias" else 1.0])
                )
...
class RepeatLatentBatch:
    ...
    def repeat(self, samples, amount):
        ...
        if "noise_mask" in samples and samples["noise_mask"].shape[0] > 1:
            ...
            if masks.shape[0] < s_in.shape[0]:
                masks = masks.repeat(
                    (math.ceil(s_in.shape[0] / masks.shape[0]),)
                    + ((1,) * (masks.ndim - 1))
                )[: s_in.shape[0]]
            s["noise_mask"] = samples["noise_mask"].repeat(
                (amount,) + ((1,) * (samples["noise_mask"].ndim - 1))
            )
        if "batch_index" in s:
        ...
        if image1.shape[-1] != image2.shape[-1]:
            if image1.shape[-1] > image2.shape[-1]:
                image2 = torch.nn.functional.pad(
                    image2, (0, 1), mode="constant", value=1.0
                )
            else:
                image1 = torch.nn.functional.pad(
                    image1, (0, 1), mode="constant", value=1.0
                )
        if image1.shape[1:] != image2.shape[1:]:
    ...
    try:
        ...
        try:
            ...
            if web_dir_name:
                ...
                if os.path.isdir(web_dir_path):
                    ...
                    logging.info(
                        "Automatically register web folder {} for {}".format(
                            web_dir_name, project_name
                        )
                    )
        except Exception as e:
            logging.warning(
                f"Unable to parse pyproject.toml due to lack dependency pydantic-settings, please run 'pip install -r requirements.txt': {e}"
            )
        ...
        # V3 Extension Definition
            ...
            if not callable(entrypoint):
                logging.warning(
                    f"comfy_entrypoint in {module_path} is not callable, skipping."
                )
                return False
            try:
                ...
                if not isinstance(extension, ComfyExtension):
                    logging.warning(
                        f"comfy_entrypoint in {module_path} did not return a ComfyExtension, skipping."
                    )
                    return False
                node_list = await extension.get_node_list()
                if not isinstance(node_list, list):
                    logging.warning(
                        f"comfy_entrypoint in {module_path} did not return a list of nodes, skipping."
                    )
                    return False
                ...
                logging.warning(
                    f"Error while calling comfy_entrypoint in {module_path}: {e}"
                )
                return False
        else:
            logging.warning(
                f"Skip {module_path} module for custom nodes due to the lack of NODE_CLASS_MAPPINGS or NODES_LIST (need one)."
            )
            return False
    except Exception as e:
    ...
    for custom_node_path in node_paths:
        ...
        for possible_module in possible_modules:
            ...
            if (
                ...
                logging.info(
                    f"Skipping {possible_module} due to disable_all_custom_nodes and whitelist_custom_nodes"
                )
                continue
...
async def init_builtin_api_nodes():
    api_nodes_dir = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "comfy_api_nodes"
    )
    api_nodes_files = [
        "nodes_ideogram.py",
        "nodes_openai.py",
    ...
    if not await load_custom_node(
        os.path.join(api_nodes_dir, "canary.py"), module_parent="comfy_api_nodes"
    ):
    ...
    for node_file in api_nodes_files:
        if not await load_custom_node(
            os.path.join(api_nodes_dir, node_file), module_parent="comfy_api_nodes"
        ):
            import_failed.append(node_file)
...
async def init_public_apis():
    register_versions(
        [
            ComfyAPIWithVersion(version=getattr(v, "VERSION"), api_class=v)
